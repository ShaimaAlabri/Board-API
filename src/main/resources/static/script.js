    ...
    async function displayBoardTitle() {
        try {
            const response = await fetch(`http://localhost:8080/api/board/1`);
            if (!response.ok) {
  ...
  function updateCustomCard(cardId) {
      const apiEndpoint = `${base_url}/card/${cardId}`; // Use the correct API endpoint
      const cardTitle = document.getElementById('updatedCardTitle').value;
      const cardDescription = document.getElementById('updatedCardDescription').value;
      const taskPriority = document.getElementById('updatedCardStatus').value;
      let cardSection;
      ...
      switch (taskPriority) {
          ...
          case "done":
              ...
              break;
          default:
              cardSection = 1; // Default to "todo" section
              break;
      }
      ...
      fetch(apiEndpoint, {
          method: 'PUT', // Use the HTTP method for updating
          headers: {
              'Content-Type': 'application/json',
          },
          body: JSON.stringify(requestBody),
      ...
      .then(updatedCard => {
          ...
          // Update the card element on the UI with the new title and description
          const selectedID = document.getElementById('IDNumber').value;
          cardElement.querySelector('.card-title').textContent = updatedCard.title;
          cardElement.querySelector('.card-description').textContent = updatedCard.description;
          cardElement.querySelector('.card-id').textContent = `ID: ${updatedCard.id}`; // Display the updated ID
